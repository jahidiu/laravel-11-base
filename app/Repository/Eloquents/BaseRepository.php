...
class BaseRepository implements BaseRepositoryInterface
{
    ...
    // {
    //     return $this->model->with($relations)->where($condition)->orderBy('id', 'desc');
    // }

    public function allDataTable(array $condition = [], array $columns = ['*'], array $relations = [], ?\Closure $customFilter = null)
    {
        $query = $this->model->with($relations)->where($condition)->select($columns)->orderBy('id', 'asc');

        if ($customFilter) {
            $customFilter($query); // Apply additional filters like whereHas
        }
